...
# Загрузка и обработка данных с использованием numpy.memmap
...
# Загрузите данные с помощью np.genfromtxt для проверки размеров
opers_data = np.genfromtxt(opers_filename, delimiter=',')
stpa_data = np.genfromtxt(stpa_filename, delimiter=',')

# Проверьте размеры данных
print("Размеры данных opers_data:", opers_data.shape)
print("Размеры данных stpa_data:", stpa_data.shape)

# Создайте memmap для обработки данных на диске
opers_memmap = np.memmap(opers_filename, dtype='float32', mode='r', shape=opers_data.shape)
stpa_memmap = np.memmap(stpa_filename, dtype='float32', mode='r', shape=stpa_data.shape)


# Создание memmap для обработки данных на диске
opers_memmap = np.memmap(opers_filename, dtype='float32', mode='r', shape=opers_data.shape)
stpa_memmap = np.memmap(stpa_filename, dtype='float32', mode='r', shape=stpa_data.shape)

# Преобразование данных в тензоры для PyTorch
X = torch.from_numpy(opers_memmap[:, :-1].reshape(opers_shape)).to(torch.float32).to(device)
y = torch.from_numpy(stpa_memmap[:, -1].reshape(stpa_shape)).unsqueeze(1).to(torch.float32).to(device)
